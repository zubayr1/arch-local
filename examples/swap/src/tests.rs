#[cfg(test)]
mod tests {
    use crate::{operations::{self, add_liquidity, claim_rewards, remove_liquidity, unstake_tokens}, setup};

    use super::*;
    use operations::{LimitOrder, OrderStatus, OrderType};

    use crate::{
        create_mock_account,
        state::LiquidityParams,
        curve::calculate_swap_amount,
        reward::RewardParams,
    };
    ...
    fn test_limit_order_creation() {
        let pubkey = Pubkey::from_slice(&[0u8; 32]);
        let owner = Pubkey::from_slice(&[1u8; 32]);
        let mut data = Box::new([0u8; 64]);
        let utxo = Box::new(UtxoMeta::from_slice(&[0u8; 36]));
        ...
        let limit_order = LimitOrder {
            owner: owner.clone(),
            token_pair: (Pubkey::from_slice(&[2u8; 32]), Pubkey::from_slice(&[3u8; 32])),
            amount: 1000,
            price: 2000,
        ...
        assert_eq!(limit_order.amount, 1000);
        assert_eq!(limit_order.price, 2000);
        assert_eq!(limit_order.order_type, OrderType::Buy);
    }

    #[test]
    fn test_process_limit_orders() {
        let pubkey = Pubkey::from_slice(&[0u8; 32]);
        let owner = Pubkey::from_slice(&[1u8; 32]);
        let mut data = Box::new([0u8; 64]);
        let utxo = Box::new(UtxoMeta::from_slice(&[0u8; 36]));
        ...
        // Market price that would execute buy order but not sell order
        let market_price = 1600;

        // Simulate order processing
        let buy_order_status = if market_price >= buy_order.price {
            OrderStatus::Executed
        } else {
            OrderStatus::Open
        };

        let sell_order_status = if market_price <= sell_order.price {
            OrderStatus::Executed
        } else {
            OrderStatus::Open
        };
        ...
        assert_eq!(buy_order_status, OrderStatus::Executed);
        assert_eq!(sell_order_status, OrderStatus::Open);
    }
}
